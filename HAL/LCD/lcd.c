...
void lcd_init(void)
{
    ...
    vTaskDelay(pdMS_TO_TICKS(20)) ;     /* LCD Power ON delay always > 15ms */
...
void lcd_send_command(uint8_t command)
{
    ...
    vTaskDelay(pdMS_TO_TICKS(1)) ; /* delay for processing Tas = 50ns */
    gpio_write_pin(LCD_E_PORT_ID,LCD_E_PIN_ID,LOGIC_HIGH); /* Enable LCD E=1 */
    vTaskDelay(pdMS_TO_TICKS(1)) ; /* delay for processing Tpw - Tdws = 190ns */
    ...
    vTaskDelay(pdMS_TO_TICKS(1)) ; /* delay for processing Tdsw = 100ns */
    gpio_write_pin(LCD_E_PORT_ID,LCD_E_PIN_ID,LOGIC_LOW); /* Disable LCD E=0 */
    vTaskDelay(pdMS_TO_TICKS(1)) ; /* delay for processing Th = 13ns */
    gpio_write_pin(LCD_E_PORT_ID,LCD_E_PIN_ID,LOGIC_HIGH); /* Enable LCD E=1 */
    vTaskDelay(pdMS_TO_TICKS(1)) ; /* delay for processing Tpw - Tdws = 190ns */
